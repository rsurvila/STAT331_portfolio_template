...
# Lab 5. In this code chunk I added the select function to remove columns which I found to be confusing when interpreting the results after running it. There were three columns that all started with id (id.x, id.y, and id.y.y) which were not helpful columns to have. 
...
inner_join(
  ...
  #don't care abut ID (there are three ID columns that could get confusing so want to get rid of them)
  select(
    -c(id.x,
       id.y,
       id.y.y)
...
#Lab 4. In this code chunk I changed the column identifiers for pivot_longer so instead of selecting multiple columns using a colon I used the function starts_with. This was seen in the first portion of the chunk before ggplot and was expertly crafted by Dr. T!
...
#Lab 5 - finding the suspect. In this chunk I filtered the date by December (month) and 2017 (year). Originally I had used str_detect to find the character 201712 but to satisfy this learning target I changed the date column to actual dates and then filtered. 
...
#Lab 3. In this code chunk I created a new variable, distinct_questions_asked, which counted the total amount of unique questions students asked in the survey. 
...
#Challlenge 4. In this code chunk I modified my data summary table to include a column of the overall median childcare price based on type (family-based or center-based) for 5 groups of overall proportion of Californians who identified as Hispanic. With the summarize line I created a new value of the overall median of childcare price, which before was the median price for each county for each year.  
...
#I used fct_relevel to relevel the Age factor so that it would plot properly in the same order as Dr. T's. This was changed from my original code.  
...
#I used fct_reorder to reorder the legend based on the census_region. I made text changes to other elements of the plot (y-axis and legend), and I reordered the facets so they went in the same order as Dr. T's. 
...
#Lab 4 Question 5. Changed the filtering to %in% instead of == upon recommendation from Dr. T then changed the order of operations to make more sense.
...
#Challenge 4.Creating a plot. In this chunk I changed my original code to theme_light to produce a plot with light grey lines and axes to direct more attention towards the data.  
...
#Challenge 4 - modifying colors. In this challenge I changed the color of my boxplots and points so that they matched the colors of an image of an elephant seal laying in grass that I found! 
...
#Challenge 4. Creating a plot. In this code chunk I added dashed lines to represent the 5 groups of percentage Hispanic Californian residents (0%-20%, 20%-40%, 40%-60%, 60%-80%, and 80%-100%). So in total there are 5 lines in the plot. This code chunk was modified from my original chunk (modified for resubmission). 
...
#Challenge 4. Creating a summary statistics table. In this code chunk I found the median of the median price of childcare across all study years using summarize. I modified this code from my original submission but not after my resumbission.
...
#Challenge 4. In this challenge I found the overall median price across California regions and census years (2008-2018). 
...
#Lab 4 Question 5. In this example I created a table (well-organized using the kable function) of the median household income for 2008 and 2018 for each of the ten California census regions. I modified my column names from 2008 and 2018 to Median_household_income_for2008 and Median_household_income_for_2018 to better communicate what the values in the column represent. This code was changed from my original code. 
...
#Challenge 4. Creating a summary statistics table. I created my table in order of least percentage of California resident who are Hispanic (0%-20%) to greatest percentage of California residents who are Hispanic (80%-100%). I modified my code from the original code I submitted.
...
#Challenge 4. Creating a summary statistics table. I used the cut function which I checked to make sure it was not supersceded in order to convert numeric variables (Hispanic proportion) to categorical with 5 groups> 0%-20%, 20%-40%, 40%-60%, 60%-80%, and 80%-100%.
