...
# Lab 9. Question 6. Growing: Can you format the percentages to have 2 decimal places and include a % symbol at the end? In this code chunk I included fmt_percent with decimals = 2 to add both the percentage sign and 2 decimal places. I also selected academic_degree, sen_level, and sex by removing the other columns in the evals dataset. 
...
#Lab 9. Question 6. In this code chunk I made a proportions column for each of the three chosen variables (seniority level, sex, and academic degree) by using the mutate function. 
...
#Lab 5 - finding the person who hired the murderer. Mutated the date column to create an actual date variable in order to filter out for January 9th.

#join gym membership dataset with check in dataset, drivers license dataset, and interview dataset
get_fit_now_member |> 
  right_join(get_fit_now_check_in, join_by(id == membership_id)) |> 
  right_join(person, join_by(person_id == id)) |> 
  right_join(drivers_license, join_by(license_id == id)) |> 
  left_join(interview, 
            by = "person_id") |> 
  #create check in date as actual date variable
  mutate(
    date = ymd(check_in_date)
  ) |> 
  #filter out for January 9th - when the interviewee saw the murderer, the gold gym membership, then plate number, and finally the membership number
  filter(
    month(date) == 01,
    day(date) == 09,
  ...
  #pull transcript from interview
  pull(transcript)
...
#Lab 5. I decided to follow in an exemplary student's footsteps and I changed my code to include anti_join so only people who had known interviews would be left. 
...
#Lab 7. Question 4. In this chunk as I was creating a function which transformed the input vector into a rescaled vector I added comments to each step. 

rescale_01 <- function(fish){
  
  #if the input vector is not numeric then the function stops and does not proceed
  if(!is.numeric(fish)){
      stop()
    }
  #if the input vector has a length greater than 1 (is not a single vector) then the function stops 
  if (length(fish) <= 1) {
      stop()
    }
  #transforms the input vector into a rescaled vector so that it only has a value from 0 to 1 
  (fish - min(fish, na.rm = TRUE)) /
           (diff(range(fish, na.rm = TRUE)))
...
#Lab 7. Question 4. In this example I created a function which makes a rescaled vector but, by using the function stops, only takes in a single vector. 
...
#Lab 2. Question 4. I created multiple scatterplots between weight and hindfoot length for various rodent species (one scatterplot per species). 
...
ggplot(data = surveys, 
       ...
       labs(x = "Weight (g))", 
            y = "",
            title = "Species plots of hindfoot length (mm) by weight (g)",
            subtitle = "Hindfoot length (mm)") + 
       facet_wrap(~species, scales = "free") +
       scale_y_continuous(limits = c(0, 65),
                          breaks = seq(from = 0, to = 65, by = 20)) + 
       theme_bw() 
...
#Lab 2 Question 10. I created a graph of how different rodent species vary by weight distribution.
...
#Challenge 3. Question 2. Growing comment: Technically, the percent() function has been superseded. Meaning, there are other functions you should use instead. Specifically, the label_percent() function has replaced this function. I changed my percent() to label_percent in the scale_y_continuous code. In this code chunk I created a plot which showed the proportion of evaluation ratings (either excellent or standard) for the three professor seniority levels. 
...
ggplot(data = teacher_evals_compare,
  ...
  theme(
    ...
    legend.position = "top") + 
  scale_fill_manual(values = c("#b890d8", "#ba955c"))
...
#Lab 3. Question 10. In this code chunk I used the summarize function to calculate the overall mean SET average score for each instructor that had at least 5 courses reviewed. 

teacher_evals_clean |> 
  group_by(teacher_id) |> 
  mutate(distinct_course_id = n_distinct(course_id)) |> 
  filter(distinct_course_id >= 5,
         question_no == 901) |> 
  summarize(average_SET_scores = mean(SET_score_avg))
...
#Lab 9 Question 6. I used the across() function to change the teacher_id and course_id column to a character type. 
...
ca_childcare |> 
  ...
  pivot_wider(names_from = study_year,
              values_from = median_household_income,
              names_prefix = "Median_household_income_for_") |>
  arrange(desc(Median_household_income_for_2018)) |> 
  kable()
...
#Lab 9. Question 6. I used column and row headers and emboldened them so that the table was easier to read. 
...
#Lab 9. Question 6. I created a table of the demographics of professors from a study and used light blue as the row header. 
...
# Lab 9. Question 6. Used pivot_longer to create a more readable table that included the three selected variables: seniority level, sex, and academic degree. 
...
#Lab 7 Question 1. In this chunk I used the across() function to summarize all of the missing values for each variable in the fish dataset. 
...
#Lab 9. Question 2. Growing: Do variables have more than one data type? I changed my code so that the enframe() function was used and got rid of the [1] right after .x.

#In this example I used a variant of the map() function (map_chr) to create a character vector to create a table of the unique types of variables found in the surveys dataset. 
...
#Lab 7. Question 4. This function took in a single numerical vector and returned a rescaled vector that was between 0 and 1.  
...
#Lab 8. Question 1. This function takes in a dataframe (data) and a vector (fish) and rescales the variables in the dataframe.
...
#Lab 8. Question 2. This function takes in a dataframe, a row variable, and a column variable, and counts the numver of observations for each combination of the two variables, pivots the results wider, and adds row and column totals. 
...
#Lab 10. Question 1. In this example I used map_int() to create a vector of integers for 10,000 simulations of a function which calculated the number of babies born in a hospital in a given night that were correctly given back to their parents. 
...
-   second example of `map()` function with **one** input

```{r}
#| label: pe-3-map-2

#Lab 9. Question 4. I used the map_if() function to convert all the columns in the evals dataset that are characters to factors.

evals |> 
  map_if(.p = is.character, .f = as_factor) |> 
  bind_cols()
...
#Lab 9. Question 6. I used the case_when() function to change the variables in the seniority level column of the evals dataset. I made sure to check that case_when() was not superseded. 
...
#I simulated 10,000 times the distribution of babies born on a single night that are correctly returned to their parents if there were 4 babies born. 

randomBabies <- function(n_babies){
  babies_born <- tibble(baby = 1:n_babies,
                        family = sample(1:n_babies,
                                        size = n_babies,
                                        replace = F))
  
  paired_family <- babies_born |>
    filter(baby == family) |> 
    nrow()
  
  return(paired_family)
...
#I simulated 1,000 times the likelihood of the population slope being contained in the estimated 95% confidence interval for the sample slope. 
...
ep = rnorm(n = n,
           mean = 0,
           sd = sqrt(1)) # SD is the square root of the variance (sigma)
...
When I originally began collaborating for the practice activities I found it difficult to be the computer and only write out the code. After understanding the roles and responsibilities of each of the two roles, I began to follow how the computer was supposed to act and let the coder go through the code while I wrote it out. I made sure to always be open to the ideas of the coder and vice versa when I was the coder. I always tried to be conscientious of who I was working with and their style of communicating. I remember for our PA 5 assignment when we were working with stringr my classmate and I had to finish outside of classtime. Although we had other tasks to accomplish outside this class, I tried to keep the energy high and be patient and respectful to create a healthy space for the both of us.

## Lightning Round Essays

-   Finish the sentence with another student’s name: "If \[Isaiah Woodard\] doesn’t earn an A, then nobody should, because he was always willing to help out his fellow classmates like myself who were having trouble with the code, as well as go above and beyond what was asked of him in lab assignments such as making a beautiful plot for lab 4. Thus, he hits on the major targets needed to achieve an A."
...
I found the functions and simulations to be the most challenging part of class because both areas were too theoretical for me. I don't come from a coding background so as we were going through the examples of how to use these functions there was no physical example to tie these areas with and they therefore felt the most code intensive.

-   What was the easiest part of the class for you? Explain why.

The easiest part of the class was learning ggplot() as I am a visual learner and had the easiest time understanding which functions from the package related to which parts of the plot. I also had the most experience in building plots in R.
